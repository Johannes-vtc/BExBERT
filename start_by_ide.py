...
class pre_train_BertTokenizer(BertTokenizer):
    ...
    def Masking(self, Input_ids, Masked_lm_labels):
        ...
        Input_ids[(rd_1 >= 0.865) * (rd_1 < 0.88)] = (
                np.random.rand(((rd_1 >= 0.865) * (rd_1 < 0.88) * 1).sum()) * len(self.vocab)).astype(int)
...
if __name__ == "__main__":
    ...
    ap.add_argument('-dv', '--device', required=True, type=int, nargs='+',
                    help='gpu id for the training, ex [-dv 0 12 3]')
    ap.add_argument('-lr', '--learning_rate', required=True, type=float, help='learning rate , google use 1e-04')
    ap.add_argument('-str', '--strategy', required=True, type=str,
                    help='choose a strategy from [exBERT], [sciBERT], [bioBERT]')
    ...
    ap.add_argument('-pm_p', '--pretrained_model_path', default=None, type=str,
                    help='path to the pretrained_model stat_dict (torch state_dict)')
    ap.add_argument('-pm_p_tf', '--pretrained_model_path_tf', default=None, type=str,
                    help='path to the pretrained_model (tensorflow .ckpt)')
    ap.add_argument('-dp', '--datat_path', required=True, type=str, help='path to data ')
    ap.add_argument('-ls', '--longest_sentence', required=True, type=int,
    ...
    ap.add_argument('-wp', '--warmup', default=-1, type=float,
                    help='portion of all training itters to warmup, -1 means not using warmup')
    ap.add_argument('-t_ex_only', '--train_extension_only', default=True, type=bool,
    ...
    if args['pretrained_model_path'] is not None:
        try:
            ...
            torch.save(model.state_dict(),"./bert_original.pt")
    else:
        bert = BertForPreTraining.from_pretrained("bert-base-uncased")
        model.load_state_dict(bert.state_dict(), strict=False)
    ...
    Train_Data, Train_Label, Val_Data, Val_Label = load_data(args['datat_path'], -1, -1, args['percentage'], 0,
                                                             load_all=True)
    ...
    try:
        for epoc in range(num_epoc):
            ...
            for sep_data_id in range(args['sep']):
                Train_Data, Train_Label, Val_Data, Val_Label = load_data(args['datat_path'], sep_data_id,
                                                                         sep_range * batch_size, args['percentage'],
                                                                         epoc)
                for batch_ind in range(int(np.ceil(len(Train_Data) / batch_size))):
                    ...
                    if batch_ind > 0 and batch_ind % print_every_ndata == 0:
                        print(
                            'Train Epoch: {} [{}/{} ({:.0f}%)]\ttrain_Loss: {:.5f} \tval_Loss: {:.5f} time: {:.4f} \t lr:{:.6f}'.format(
                                epoc,
                                batch_ind * batch_size + sep_data_id * sep_range * batch_size,
                                all_data_num,
                                100 * (batch_ind * batch_size + sep_data_id * sep_range * batch_size) / all_data_num,
                                train_loss / print_every_ndata / batch_size, val_loss / print_every_ndata / batch_size,
                                time.time() - t2,
                                optimizer.get_lr()[0]))
                        train_loss = 0
                        val_loss = 0
                ...
                if len(device_ids) > 1:
                    t.save(model.module.state_dict(),
                           args['save_path'] + '/state_dic_' + args['strategy'] + '_' + str(epoc))
                else:
